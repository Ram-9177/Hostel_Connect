...
export class NotificationsController {
  ...
  async sendNotification(@Request() req, @Body() notificationData: any) {
    ...
    if (!['SUPER_ADMIN', 'WARDEN_HEAD'].includes(req.user.role)) {
      throw new ForbiddenException('Unauthorized to send notifications');
    }
  ...
  @Post('send-targeted')
  @ApiOperation({ summary: 'Send targeted notification to specific groups (Admin/Warden only)' })
  @ApiResponse({ status: 201, description: 'Targeted notification sent successfully' })
  async sendTargetedNotification(
    @Request() req,
    @Body() createNotificationDto: CreateTargetedNotificationDto,
  ) {
    // Check permissions
    if (!this.checkAdminPermission(req.user.role)) {
      throw new ForbiddenException('Only admins and wardens can send targeted notifications');
    }

    const result = await this.notificationsService.createTargetedNotification(
      createNotificationDto,
      req.user.id,
    );

    // Send real-time notifications via WebSocket
    // Note: This will be handled by the socket service for all targeted users
    ...
    return {
      success: true,
      message: `Notification sent to ${result.targetCount === -1 ? 'role-based users' : result.targetCount + ' students'}`,
